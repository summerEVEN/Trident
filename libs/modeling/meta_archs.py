...
class ClsHead(nn.Module):
    ...
    def __init__(
        ...
        feat_dim,   # 特征维度
        num_classes,    # 类别数量
        prior_prob=0.01,   # 先验概率，默认为0.01
        num_layers=3,  # 模型层数，默认为3
        kernel_size=3, # 卷积核大小，默认为3
        act_layer=nn.ReLU,  # 激活函数，默认为ReLU
        with_ln=False,  # 是否使用层归一化，默认为False
        empty_cls=[],   # 空类别列表，默认为空
        detach_feat=False  # 是否分离特征，默认为False
    ):
        super().__init__()
        self.act = act_layer()
...
class TriDet(nn.Module):
    ...
    def decode_offset(self, out_offsets, pred_start_neighbours, pred_end_neighbours):
        ...
        if not self.use_trident_head:
            ...
            if self.training:
                ...
                # 前两个维度不变，第三个维度修改为2，最后一个维度自适应
                pred_start_neighbours = torch.cat(pred_start_neighbours, dim=1)
                # torch.cat函数，除了需要拼接的维度可以不一样之外，其他维度必须一样
                # pred_start_neighbours 是一个张量列表list，里面包含2个以上的张量
            ...
            ## 把 pred_right_dis 和 pred_right_dis 里面非法值，替换成0，确保张量里面不含NaN值
    ...
    def forward(self, video_list):
        ...
        # out_cls_logits, out_lb_logits, out_rb_logits: `torch.Size([2, 20, 2304])
        ...
        # out_offset: List[B, 2, T_i]  [2, 34, 2304]
        ...
        if self.training:
            ...
            # 合并查看结果的代码
            s = results[0]["scores"].unsqueeze(dim=-1)
            la = results[0]["labels"].unsqueeze(dim=-1)
            show = torch.cat([results[0]["segments"], s, la], dim=1)
...
# 这个函数 label_points_single_video 用于为单个视频的时间点列表、标签片段和标签标签进行标记，生成类别目标和回归目标。
    @torch.no_grad()
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        # concat_points : F T x 4 (t, regressoin range, stride) （许许多多个四元组）
        # gt_segment : N (#Events) x 2      （片段的起始时间和结束时间点）
        # gt_label : N (#Events) x 1        （片段的标签）
        num_pts = concat_points.shape[0]
        num_gts = gt_segment.shape[0]

        # corner case where current sample does not have actions
        # 如果没有标签片段，则创建全零的类别目标张量和全零的回归目标张量
        if num_gts == 0:
        ...
        # 计算了每个真实动作（或段）的持续时间，它通过减去动作的结束时间与开始时间来计算每个动作的持续时间
        # 扩展了持续时间的张量，使其具有与模型处理的所有时间点相匹配的形状
        lens = gt_segment[:, 1] - gt_segment[:, 0]
        lens = lens[None, :].repeat(num_pts, 1)
        """
        具体地，它通过将 None 索引添加到 lens 张量的维度中，使其成为一个二维张量，
        然后使用 repeat 方法将其沿着第一个维度（行）复制 num_pts 次，以匹配 concat_points 的形状。
        这样，每个时间点上都有一个持续时间值，而 num_pts 表示时间点的数量。        num_pts: 4536 (2304 + 2304/2 + 2304/4 + ……)        lens.shape: [4536, 3]     3是根据数据集里面的时间段决定的
        """

        # compute the distance of every point to each segment boundary
        # auto broadcasting for all reg target-> F T x N x2
        # 这几行代码计算了每个时间点与每个动作（或“段”）边界之间的距离，以便用于后续的目标回归计算
        gt_segs = gt_segment[None].expand(num_pts, num_gts, 2)
        left = concat_points[:, 0, None] - gt_segs[:, :, 0]
        right = gt_segs[:, :, 1] - concat_points[:, 0, None]
        reg_targets = torch.stack((left, right), dim=-1)
        # 计算了某个点，距离这个段的左边起点，以及右边结束点的距离
        # 具体是什么距离还是很模糊

        if self.train_center_sample == 'radius':
            """
            center_pts 计算了每个动作片段的中心点，这是通过对每个动作片段的起始和结束位置取平均来实现的。
            然后，根据给定的中心采样半径，计算了新的采样范围。
            这是通过将中心点减去（左边界）或加上（右边界）一定的距离来实现的，
            这个距离是通过动作片段的采样步长（concat_points[:, 3]）和
            中心采样半径（self.train_center_sample_radius）来计算的。
            """
    ...
    def losses(
        ...
        """ 参数解释：
            fpn_masks,  掩码（确定需要处理的时间范围）
            out_cls_logits, 分类的softmax概率
            out_offsets,    最后的偏移概率分布
            gt_cls_labels,  ground_turth 的分类标签
            gt_offsets,     ground_turth 的偏移数据
            out_start,      片段起始点的概率
            out_end,        片段结束点的概率
        ...
        if self.use_trident_head:
            ...
            for i in range(len(out_start)):
                ...
                # 将张量 x 的大小（即形状）转换为列表。这个列表包含了张量 x 每个维度的尺寸信息
                ...
                # 创建一个张量的视图，而不是分配新的内存空间 （as_strided函数）
                # 创建一个与原始张量 x 具有不同形状的视图，但在特定维度上具有相同的步幅
                x = x.as_strided(size=x_size, stride=x_stride)
                out_start_logits.append(x.permute(0, 2, 1, 3))
                # (0, self.num_bins)： 0表示左边填充0个指定的数字，self.num_bins表示右边填充这么多个指定的数字
                # mode='constant', value=0：填充方式是常数，数值为0
                # unsqueeze(-1)： 在最后一个维度上添加一个维度
        ...
        # 找到里面的正例数据？？
        # torch.stack(): 把gt_cls_labels里面所有的张量进行堆叠
        # 里面每一个张量的维度都是一样的
        # 没有指定dim参数，默认dim为0
        # 第0维将会变成张量的个数
        ...
        # decoded_offsets = decoded_offsets[pos_mask] 
        # decoded_offsets和pos_mask的维度应该一样
        # 这一步会把pos_mask对应位置上的元素删除，后面的元素会补上来，特征的维度也会变化
        # 试过使用二维tensor，最后结果都变成了一维的tensor
        # 这一步应该是把之前为了时间对齐，把start和end补上的0给去掉了？
    ...
    def inference(
        ...
        for idx, (vidx, fps, vlen, stride, nframes) in enumerate(
            ...
            # cls_logits_per_vid 这个参数里面包含了多层下采样预测的每一帧的结果，感觉这个可以用来当作计算acc的数值
            # 然后在此基础上，关于那个slowfast的训练，也就是那个损失函数，也许可以换一下，提取特征的方法也需要进一步处理？
    ...
    def postprocessing(self, results):
        ...
        for results_per_vid in results:
            ...
            if self.test_nms_method != 'none':
                # 他在输入这个函数的时候，seg表示识别到的许许多多个检测片段结果[strat_time, end_time]
                # scores表示每一个结果的置信度分数，labels表示每一个片段的预测标签
