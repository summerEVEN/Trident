...
class HacsDataset(Dataset):
    def __init__(
        ...
        self.db_attributes = {
            ...
            # 修改iou返回的代码             # 'tiou_thresholds': np.array([0.5, 0.75, 0.95]),
            'empty_label_ids': []
        }
    ...
    def __getitem__(self, idx):
        ...
        if self.backbone_type == 'i3d':
            ...
            if self.backbone_type == 'slowfast':
                ...
                feats = torch.concat([feats[0], feats[1]], dim=1).squeeze(0).transpose(0, 1)
            elif self.backbone_type == 'tsp':
                filename = os.path.join(self.feat_folder, 'v_' + video_item['id'] + self.file_ext)
                feats = np.load(filename, allow_pickle=True)

        # we support both fixed length features / variable length features 我们支持固定长度特征/可变长度特征
        ...
        # convert time stamp (in second) into temporal feature grids  将时间戳(秒)转换为时间特征网格
